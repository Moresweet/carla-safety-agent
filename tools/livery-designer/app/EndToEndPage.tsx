...
export default function EndToEndPage(){
 ...
 return <section className="e2e-page">
  <aside className="e2e-sidebar"><span className="eyebrow">ALGORITHM CATALOG</span><h2>End-to-End Testing</h2><p>Choose an installed target. Missing checkpoints are shown but cannot run.</p>{status?.algorithms.map(a=><button key={a.id} className={`algorithm-card ${algorithm===a.id?"selected":""}`} disabled={!a.enabled} onClick={()=>setAlgorithm(a.id)}><div className="algorithm-mark">{a.name[0]}</div><div><strong>{a.name}</strong><small>{a.framework} · {a.sensors}</small></div><span>{a.enabled?"READY":"NO WEIGHT"}</span></button>)}</aside>
  <div className="e2e-main">
   ...
   <div className="preset-panel"><div className="stagebar"><span>PRESET SCENARIOS</span><em>BENCH2DRIVE · EXTERNAL READ ONLY</em></div><div><label>Route collection</label><select value={collection} onChange={e=>{setCollection(e.target.value);setPreset(collections.find(v=>v.path===e.target.value)?.routes[0]?.id||"")}}>{collections.map(v=><option key={v.path} value={v.path}>{v.name} · {v.route_count} routes</option>)}</select></div><div><label>Town · route · scenarios</label><select value={preset} onChange={e=>setPreset(e.target.value)}>{selectedCollection?.routes.map(v=><option key={v.id} value={v.id}>{v.town} · #{v.id} · {v.scenarios.join(", ")}</option>)}</select></div><button className="primary" disabled={running||!preset||!selectedAlgorithm?.enabled} onClick={()=>start("preset-run")}>Run preset scene</button></div>

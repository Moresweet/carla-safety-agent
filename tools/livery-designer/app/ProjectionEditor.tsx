...
export default function ProjectionEditor({mesh,image,output,onRender}:Props){
 ...
 const [view,setView]=useState<View>("left"),[x,setX]=useState(50),[y,setY]=useState(50),[scale,setScale]=useState(46),[rotation,setRotation]=useState(0);
 useEffect(()=>{
  ...
  const layer=document.createElement("canvas");layer.width=layer.height=size;const lc=layer.getContext("2d")!;if(image){const w=scale/100*frame.w,h=w*image.height/image.width;lc.translate(frame.x+x/100*frame.w,frame.y+y/100*frame.h);lc.rotate(rotation*Math.PI/180);lc.drawImage(image,-w/2,-h/2,w,h)}
 ...
 },[mesh,image,output,onRender,view,x,y,scale,rotation]);

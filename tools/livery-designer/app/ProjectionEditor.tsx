...
export default function ProjectionEditor({mesh,image,output,onRender}:Props){
 ...
 useEffect(()=>{
  ...
  const layer=document.createElement("canvas");layer.width=layer.height=size;const lc=layer.getContext("2d")!;if(image){const w=scale/100*frame.w,h=w*image.height/image.width;lc.translate(frame.x+x/100*frame.w,frame.y+y/100*frame.h);lc.rotate(rotation*Math.PI/180);if(mode==="decal")lc.drawImage(image,-w/2,-h/2,w,h);else if(mode==="tile"){for(let py=-size*1.5;py<size*1.5;py+=Math.max(20,h))for(let px=-size*1.5;px<size*1.5;px+=Math.max(20,w))lc.drawImage(image,px,py,w,h)}else{const points=[[0,0],[-.58,-.4],[.58,-.4],[-.58,.4],[.58,.4]];for(const [px,py] of points)lc.drawImage(image,-w*.34+px*w,-h*.34+py*h,w*.68,h*.68)}}

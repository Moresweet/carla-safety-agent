...
export default function CarPreview({mesh,atlas,revision,useSourceUV=false,label="3D BODY PREVIEW"}:{mesh:MeshData|null;atlas:RefObject<HTMLCanvasElement|null>;revision:number;useSourceUV?:boolean;label?:string}){
 ...
 useEffect(()=>{
  ...
  for(let i=0,j=0,k=0;i<raw.length;i+=15){
   ...
   for(let q=0;q<3;q++){const point=[a,b,d][q];positions.set(point,j);let px:number,py:number;if(useSourceUV){px=raw[i+q*5+3];py=raw[i+q*5+4]}else if(!side){px=norm(point[0],0)/3;py=1-norm(point[1],1)}else if(!right){px=(1+norm(point[0],0))/3;py=1-norm(point[2],2)}else{px=(2+norm(point[0],0))/3;py=1-norm(point[2],2)}uvs.set([px,py],k);j+=3;k+=2}
  }
 ...
 return <div className="preview"><div className="previewbar"><b>{label}</b><span>Drag to orbit · live texture feedback</span></div><canvas ref={canvas} onPointerDown={e=>{e.currentTarget.setPointerCapture(e.pointerId);drag.current={x:e.clientX,y:e.clientY,...angle.current}}} onPointerMove={move} onPointerUp={()=>drag.current=null}/><div className="axis"><i className="front"/>Front<i className="left"/>Sides<i className="top"/>Roof</div></div>
}

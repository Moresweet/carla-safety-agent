...
export default function Home() {
  ...
  return <main><header><div><span className="eyebrow">CARLA SAFETY AGENT</span><h1>Vehicle Livery Studio</h1></div><div className="status"><i/>Local workspace</div></header>
    ...
    <aside className="export"><span className="eyebrow">EXPERIMENTAL OUTPUT</span><h2>Send to CARLA</h2><p>Updates the existing Tesla bodywork texture parameter. Full-color live livery remains disabled until CARLA's tagged skeletal proxy supports safe material replacement.</p><button className="primary" disabled={!image||applying} onClick={apply}>{applying?"Applying…":"Update bodywork parameter"}</button><small>{applyState}</small><button disabled={!image} onClick={png}>Export texture PNG</button><button disabled={!image} onClick={config}>Export configuration</button><dl><div><dt>Target</dt><dd>Tesla Model 3</dd></div><div><dt>Transport</dt><dd>Local live bridge</dd></div><div><dt>Resolution</dt><dd>2048 × 2048</dd></div></dl></aside></section></main>;
}
